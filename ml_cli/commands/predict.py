...
@click.command(
    help="""Make predictions on new data using a trained model.
...
def predict(input_path: str, output_path: str, model_path: str):
    ...
    try:
        ...
        click.secho(
            f"Error: One or more columns required by the model are not present in the input data. Missing columns: {e}",
            fg="red",
        )
        logging.error(
            f"Error: One or more columns required by the model are not present in the input data. Missing columns: {e}"
